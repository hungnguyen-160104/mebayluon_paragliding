...
function buildSheets(
  ...
  const cover: SheetSpec = {
    ...
    ["Kỳ báo cáo", `${formatDateKeyVN(summary.from)} – ${formatDateKeyVN(summary.to)}`],
      ...
      ["Bay kéo cờ đỏ/cờ sinh nhật", t.flagFlight],
  ...
  const payroll: SheetSpec = {
    ...
    rows: [
      ...
      [
        ...
        summary.byPilot.reduce((s, p) => s + p.mealBreakfast, 0),
        summary.byPilot.reduce((s, p) => s + p.mealLunch, 0),
        summary.byPilot.reduce((s, p) => s + p.mealDinner, 0),
        summary.byPilot.reduce((s, p) => s + p.motorbikeRides, 0),
  ...
  const days: SheetSpec = {
    ...
    header: [
      ...
      "Flycam (KT)", "Camera man báo", "360 (KT)", "Phi công báo 360", "Bay kéo cờ đỏ/cờ sinh nhật",
  ...
  const pilotDaily: SheetSpec = {
    ...
    widths:
      summary.spot === "ha-noi"
        ? [12, 24, 10, 10, 10, 9, 8, 9, 9, 11, 14, 12, 14, 13, 13, 13, 12, 14, 12, 40, 24]
        : [12, 24, 10, 10, 10, 9, 8, 9, 9, 11, 14, 12, 14, 12, 14, 12, 40, 24],
