// app/api/baocao/penalty/route.ts
import { NextResponse } from "next/server";

import { isDateKey } from "@/lib/baobay/date";
import { resolveSpot } from "@/lib/baobay/request-spot";
import { requireBaobay } from "@/middlewares/requireBaobay";
import { BaobayError, getLatePenaltyStatus, setSubmitDeadline, waiveLatePenalty } from "@/services/baobay.service";
 ...
 * PATCH {deadline: "HH:MM"}            -> kế toán đặt giờ phạt cho điểm này
 *
...
export async function PATCH(req: Request) {
  const auth = requireBaobay(req, { roles: ["accountant", "admin"] });
  if (auth instanceof NextResponse) return auth;
  ...
  try {
    const deadline = await setSubmitDeadline(auth, spot, String(body?.deadline ?? ""));
    return NextResponse.json({ deadline });
  } catch (err) {
    if (err instanceof BaobayError) {
      return NextResponse.json({ message: err.message }, { status: err.status });
    }
    console.error("PATCH /api/baocao/penalty error:", err);
    return NextResponse.json({ message: "Không đặt được giờ phạt" }, { status: 500 });
  }
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    console.error("POST /api/baocao/penalty error:", err);
