// app/api/admin/baocao/accounts/route.ts
...
export async function GET(req: Request) {
  ...
  return NextResponse.json({ accounts: await listAccounts(auth), adminLevel: auth.viaAdmin ? 1 : auth.adminLevel ?? 2 });
}
...
export async function POST(req: Request) {
  ...
  if (body?.mode === "bulk") {
    ...
    if (role === "admin" && !isFullAdmin(auth)) {
      return NextResponse.json(
        { message: "Quản trị cấp 2 không được lập tài khoản quản trị" },
        { status: 403 },
      );
    }
