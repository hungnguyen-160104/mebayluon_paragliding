// app/api/baobay/export/route.ts
...
function buildSheets(
  ...
  const cover: SheetSpec = {
    ...
    ["Kỳ báo cáo", `${formatDateKeyVN(summary.from)} – ${formatDateKeyVN(summary.to)}`],
      ...
      ["Khách ngoại giao", t.diplomaticGuests],
      [],
      [
        "Lưu ý",
  ...
  const payroll: SheetSpec = {
    ...
    header: [
      ...
      "Chi đã ứng (hoàn lại)",
      "Phạt nộp muộn (trừ)",
      "Còn lại sau chi & phạt",
    ],
    widths: [26, 16, 12, 12, 10, 12, 10, 12, 16, 20, 20, 22],
    rows: [
      ...
      [
        ...
        summary.byPilot.reduce((s, p) => s + p.expenseTotal - p.latePenalty, 0),
      ],
      [],
      [
        "Ghi chú",
        "Cột “Còn lại sau chi & phạt” = tiền ứng được hoàn − tiền phạt. Đơn giá chuyến/dịch vụ do kế toán nhân thêm bên ngoài, app không giữ đơn giá.",
      ],
    ],
  };
  ...
  const handoverSheet: SheetSpec = {
    ...
    header: ["Ngày", "Người đưa", "Vai trò", "Số tiền", "Hình thức", "Nội dung", "Trạng thái", "Thời điểm xác nhận", "Người xác nhận"],
    widths: [12, 26, 14, 16, 14, 34, 16, 22, 18],
    rows: [
      ...handovers.map((h) => [
        h.date,
        h.staffName,
      ...
      [
        "TỔNG ĐÃ NHẬN", "", "",
        handovers.filter((h) => h.confirmed).reduce((s, h) => s + h.amount, 0),
        "", "",
        `${handovers.filter((h) => h.confirmed).length}/${handovers.length} khoản`, "", "",
      ],
      [
        "CÒN CHỜ XÁC NHẬN", "", "",
        handovers.filter((h) => !h.confirmed && !h.rejected).reduce((s, h) => s + h.amount, 0),
        "", "", "", "", "",
      ],
    ],
  };

  return [cover, payroll, days, pilotDaily, dispatcher, cameraman, expenses, handoverSheet];
}
