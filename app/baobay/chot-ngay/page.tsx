...
function DailyCloseInner() {
      ...
      try {
        apply(
          await apiGet<{ close: DailyCloseDTO | null; reconcile: ReconcileDTO }>(
            `/api/baobay/close?date=${targetDate}&spot=${spot}`,
          ),
        );
      } catch (err: any) {
  ...
  async function action(kind: "save" | "close" | "reopen") {
    ...
    try {
      ...
      if (kind === "close") {
        const res = await apiPost<{ close: DailyCloseDTO; reconcile: ReconcileDTO }>(`/api/baobay/close?spot=${spot}`, {
          action: "close",
          date,
        });
        apply(res);
