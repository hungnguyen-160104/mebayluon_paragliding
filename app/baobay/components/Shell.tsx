// app/baobay/components/Shell.tsx
"use client";
...
export function Shell({
  ...
  const tabs =
    user.role === "accountant"
      ? [
          { href: "/baobay/chot-ngay", label: "Chốt ngày" },
          { href: "/baobay/tong-hop", label: "Tổng hợp" },
          { href: "/baobay/bao-cao-thang", label: "Báo cáo tháng" },
        ]
      : [];
  ...
  async function logout() {
    setLoggingOut(true);
    try {
      await apiPost("/api/baobay/logout");
    } finally {
      router.replace("/baobay");
    }
  }
...
/** Khung tự đổi mật khẩu — export để trang /baobay/admin (không dùng Shell) dùng lại. */
export function ChangePasswordCard({ onDone }: { onDone: () => void }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      await apiPost("/api/baobay/password", { currentPassword, newPassword });
      setDone(true);
      setCurrent("");
      setNext("");
      setConfirm("");
