// app/baobay/components/PenaltyCard.tsx
"use client";
...
export function PenaltyCard({ spot, date, reloadKey }: { spot: string; date: string; reloadKey?: number }) {
  ...
  const load = useCallback(async () => {
    try {
      setData(await apiGet<PenaltyStatus>(`/api/baobay/penalty?date=${date}&spot=${spot}`));
      setError(null);
    } catch (err: any) {
  ...
  async function toggle(row: PenaltyRow) {
    ...
    setError(null);
    try {
      await apiPost(`/api/baobay/penalty?spot=${spot}`, {
        date,
