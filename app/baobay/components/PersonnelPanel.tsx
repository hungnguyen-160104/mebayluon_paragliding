// app/baobay/components/PersonnelPanel.tsx
"use client";
 ...
 *  - /baobay/admin   : tài khoản vai trò "Quản trị" đăng nhập cổng /baobay (cookie)
 *  - /admin/baobay   : chủ website vào bằng token quản trị (Bearer)
...
export function PersonnelPanel() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await api<{ accounts: BaobayAccountDTO[] }>("/api/admin/baobay/accounts", {
        headers: authHeader(),
      });
    ...
    <div className="space-y-6">
      <header>
        ...
        <p className="mt-1 text-sm text-slate-600">
          ...
          <a href="/baobay" className="font-medium text-sky-700 underline" target="_blank" rel="noreferrer">
            mebayluon.com/baobay
          </a>
          .
        </p>
      </header>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <SingleCreateCard onCreated={addCredentials} />
        <BulkCreateCard onCreated={addCredentials} />
      </div>
...
function SpotSettingsCard() {
  ...
  const load = useCallback(async () => {
    try {
      const r = await api<{ settings: SpotSettingDTO[] }>("/api/admin/baobay/settings", { headers: authHeader() });
      setRows(r.settings);
      setDraft(
        Object.fromEntries(
  ...
  async function save(spot: SpotId) {
    ...
    setMessage(null);
    try {
      await api("/api/admin/baobay/settings?spot=" + spot, {
        method: "PUT",
        headers: authHeader(),
        body: JSON.stringify({
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-3">
        {rows.map((row) => (
          <div key={row.spot} className="rounded-lg border border-slate-200 p-4">
            ...
            <Button size="sm" className="mt-3" disabled={busy === row.spot} onClick={() => save(row.spot)}>
...
function HandoverCard() {
  ...
  const load = useCallback(async () => {
    try {
      const r = await api<{ handovers: HandoverRowDTO[]; pendingBySpot: Record<string, number> }>(
        `/api/admin/baobay/handovers?spot=${spot}`,
        { headers: authHeader() },
      );
      setRows(r.handovers);
  ...
  async function act(row: HandoverRowDTO, reject?: string) {
    ...
    setError(null);
    try {
      await api(`/api/admin/baobay/handovers?spot=${spot}`, {
        method: "POST",
        headers: authHeader(),
          ...
          <ul className="mt-1 divide-y divide-amber-100 rounded-lg border border-amber-200">
            {pending.map((r) => (
              <li key={r.id} className="flex flex-wrap items-center gap-2 bg-amber-50/50 px-3 py-2 text-sm">
                ...
                <span className="text-xs text-slate-500">→ {r.recipientName}</span>
...
function PeriodTotalsCard() {
  ...
  const load = useCallback(async (f: string, t: string) => {
    ...
    try {
      setData(
        await api<BaobaySummaryDTO>(`/api/baobay/summary?from=${f}&to=${t}&spot=${spot}`, {
          headers: authHeader(),
        }),
      );
    } catch (err: any) {
...
function SingleCreateCard({ onCreated }: { onCreated: (items: NewCredential[]) => void }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setMessage(null);
    try {
      const res = await api<{ account: BaobayAccountDTO; password: string }>("/api/admin/baobay/accounts", {
        method: "POST",
        headers: authHeader(),
...
function BulkCreateCard({ onCreated }: { onCreated: (items: NewCredential[]) => void }) {
  ...
  async function patch(body: Record<string, unknown>) {
    ...
    try {
      const res = await api<{ account: BaobayAccountDTO; password?: string }>(
        `/api/admin/baobay/accounts/${account.id}`,
