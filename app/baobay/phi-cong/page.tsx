...
export default function PilotReportPage() {
  ...
  const loadDay = useCallback(async (targetDate: string) => {
    ...
    try {
      ...
      }>(`/api/baobay/reports/pilot?date=${targetDate}&spot=${spot}`);
  ...
  const loadHistory = useCallback(async () => {
    if (!spot) return;
    try {
      const { reports } = await apiGet<{ reports: PilotReportDTO[] }>(`/api/baobay/reports/pilot?spot=${spot}`);
      setHistory(reports);
    } catch {
  ...
  async function save(submit: boolean) {
    ...
    try {
      const res = await apiPost<{ report: PilotReportDTO; warnings: string[]; check: DayCheck }>(
        `/api/baobay/reports/pilot?spot=${spot}`,
        {
          date,
          ...form,
