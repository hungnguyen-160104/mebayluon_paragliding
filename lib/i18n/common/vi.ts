export type { CommonTranslation } from "./schema";
import type { CommonTranslation } from "./schema";

export const vi: CommonTranslation = {
  nav: {
    home: "Trang chủ",
    about: "Về chúng tôi",
    spots: "Điểm bay",
    ppg: "Dù máy (PPG)",
    pilots: "Phi công",
    homestay: "Homestay & Café",
    booking: "Đặt bay",
    preNotice: "Chuẩn bị trước khi bay",
    contact: "Liên hệ",
    bookNow: "Đặt ngay",
    login: "Đăng nhập",
    blog: "Blog",
    store: "Cửa hàng",
    knowledge: "Học bay dù lượn",
  },

  hero: {
    title: "MEBAYLUON PARAGLIDING",
    subtitle: "Fly Above Vietnam",
    description: "Trải nghiệm bay tự do giữa mây trời Việt Nam",
    bookNow: "Đặt Lịch Bay Ngay",
    learnMore: "Tìm hiểu thêm",
  },

  about: {
    title: "VỀ CHÚNG TÔI",
    subtitle: "Mebayluon Paragliding – nơi những giấc mơ bay cao",
    description: [
      "Tự hào là đơn vị bay dù lượn chuyên nghiệp đầu tiên tại Việt Nam với đội ngũ phi công có nhiều giờ bay nhất Việt Nam.",
      "Chúng tôi đồng hành cùng bạn trong những chuyến bay đầy cảm xúc.",
      "Cam kết mọi trang thiết bị bay đều đạt tiêu chuẩn quốc tế và luôn mang đến trải nghiệm bay tự do và đáng nhớ nhất cho bạn.",
    ],
  },

  spots: {
    title: "ĐIỂM BAY NỔI BẬT",
    subtitle: "Khám phá vẻ đẹp Việt Nam từ trên cao với những điểm bay tuyệt nhất",
    viewDetails: "Xem Chi Tiết",
    locations: {
      muongHoaSapa: {
        name: "Sapa",
        location: "Lào Cai",
        duration: "Tour 90 phút",
        highlights: [
          "Ngắm Sapa từ trên cao",
          "View đỉnh Fansipan",
          "Bay trên biển mây",
        ],
        summary:
          "Bay cao hàng ngàn mét ngắm trọn Sapa từ trên cao - view ruộng bậc thang và ngắm nhìn đỉnh Fansipan. Điểm bay chỉ cách trung tâm Sapa 10 phút.",
      },
      sonTra: {
        name: "Sơn Trà",
        location: "Đà Nẵng",
        duration: "Tour 60 phút",
        highlights: [
          "Ngắm toàn cảnh Đà Nẵng",
          "Bay trên biển",
          "Bay từ đỉnh Sơn Trà",
        ],
        summary:
          "Cất cánh từ đỉnh Sơn Trà bay qua bãi biển - ngắm toàn cảnh Đà Nẵng từ trên cao. Nổi tiếng view hoàng hôn biển tuyệt đẹp.",
      },
      khauPha: {
        name: "Đèo Khau Phạ",
        location: "Tú Lệ - Mù Cang Chải",
        tagline: "Điểm bay Dù lượn & Dù lượn gắn động cơ",
        duration: "Tour 60 phút",
        highlights: [
          "Bay trên tứ đại đỉnh đèo",
          "Điểm bay trên mùa vàng",
          "Bay săn mây trên 2.000m",
        ],
        summary:
          "Một trong những điểm bay đẹp nhất Thế giới. Bay trên đèo Khau Phạ hùng vĩ, một trong Tứ đại đỉnh đèo Việt Nam. Điểm Bay trên mùa vàng, bay trên mùa nước đổ và Bay săn mây lên 2.000m (Paramotor).",
      },
      tramTau: {
        name: "Phình Hồ - Trạm Tấu",
        location: "Lào Cai",
        duration: "Tour 90 phút",
        highlights: ["Bay trên cánh đồng Mường Lò", "Bay trên biển mây"],
        summary:
          "Điểm bay nằm ngay gần Nghĩa Lộ, ngắm toàn cảnh cánh đồng Mường Lò. Nơi đây rất nổi tiếng với biển mây.",
      },
      vienNam: {
        name: "Bắc Sum | Quản Bạ",
        location: "Hà Giang",
        duration: "Tour 90 phút",
        highlights: [
          "Checkin Hà Giang từ độ cao hàng ngàn mét",
          "Ngắm nhìn toàn cảnh Cao nguyên đá",
        ],
        summary:
          "Ngắm cao nguyên đá Hà Giang tuyệt đẹp từ độ cao hàng ngàn mét, một góc nhìn hoàn toàn mới.",
      },
      doiBu: {
        name: "Đồi Bù | Viên Nam",
        location: "Hà Nội",
        duration: "Tour 3 - 5 giờ",
        highlights: [
          "Điểm bay gần thủ đô",
          "Bay trên đồi cỏ lau",
          "Bay ngắm hoàng hôn",
        ],
        summary:
          "Điểm bay ở ngoại thành Hà Nội, đi về chỉ trong 3-5 giờ. Nổi tiếng với cảnh đồi cỏ lau trắng, có thể kết hợp cắm trại và trekking.",
      },
    },
  },

  features: {
    title: "Tại sao chọn Mebayluon?",
    safety: {
      title: "Thể thao mạo hiểm",
      description: "Thiết bị hiện đại, phi công giàu kinh nghiệm",
    },
    professional: {
      title: "Chuyên nghiệp",
      description: "Đội ngũ phi công được đào tạo quốc tế",
    },
    experience: {
      title: "Trải nghiệm tuyệt vời",
      description: "Cảm giác bay lượn không thể quên",
    },
    service: {
      title: "Phục vụ tận tâm",
      description: "Hỗ trợ khách hàng 24/7",
    },
  },

  cta: {
    title: "Sẵn sàng bay cùng chúng tôi?",
    subtitle: "Đặt tour ngay hôm nay và trải nghiệm Việt Nam từ trên cao",
    button: "Đặt Tour Ngay",
  },

  preNotice: {
    title: "Chuẩn bị trước khi bay",
    subtitle: "Đọc kỹ thông tin quan trọng để có chuyến bay thú vị và trọn vẹn",

    preparation: {
      title: "Chuẩn bị trước khi bay",

      clothing: {
        title: "Trang phục",
        items: [
          "Trang phục: Trang phục thoải mái, thể thao (áo tay dài, quần dài); không mặc váy",
          "Giày: Không mang giày cao gót; ưu tiên giày thể thao hoặc giày leo núi. Có hỗ trợ mượn giày miễn phí khi cần",
          "Kính mát: Kính râm để chống tia UV và gió mạnh (30–40 km/h khi bay). Có thể mang theo kính cận.",
          "Phụ kiện: Có thể mang theo 1 túi nhỏ (1–2kg) đựng đồ cá nhân như điện thoại, chìa khóa, giấy tờ tùy thân",
        ],
      },

      items: {
        title: "Quy trình bay",
        list: [
          "Tại điểm cất cánh, làm quen với phi công, nghe hướng dẫn và đặt câu hỏi",
          "Mặc trang bị bay, tập các động tác cất cánh",
          "Chạy đà mạnh, liên tục khi cất cánh theo hướng dẫn của phi công",
          "Thư giãn, ngắm cảnh và trò chuyện khi đã bay lên không trung",
          "Trang bị bay an toàn, thoải mái",
          "Hạ cánh nhẹ nhàng, có thể đứng hoặc ngồi tùy điều kiện",
        ],
      },

      transport: {
        title: "Xe đưa đón",
        items: [
          "Có dịch vụ đón và trả khách từ khách sạn theo thông tin đặt vé.",
          "Đối với người đi cùng (không bay), vui lòng thông báo số lượng để sắp xếp và báo phí phù hợp.",
        ],
      },
    },

    posters: {
      title: "Lưu ý quan trọng",
      subtitle:
        "Tóm tắt các quy định & chuẩn bị cần thiết. Nhấp vào ảnh để xem bản lớn.",
    },

    requirements: {
      title: "Điều kiện quy định đối với hành khách",

      eligible: {
        title: "Điều kiện tham gia bay",
        items: [
          "Cân nặng: Dưới 120kg. Trường hợp trên 90kg hoặc dưới 30kg vui lòng thông báo trước để sắp xếp phi công và trang thiết bị phù hợp.",
          "Thể lực: Mức độ thể lực cơ bản, có khả năng chạy ngắn. Không phù hợp với người thừa cân nhiều hoặc gặp vấn đề nghiêm trọng về vận động.",
          "Độ tuổi: Từ 3 tuổi trở lên",
        ],
      },

      notEligible: {
        title: "Đặt vé",
        items: [
          "Đặt vé trực tiếp qua website hoặc liên hệ hotline/Zalo/WhatsApp",
          "Thanh toán bằng tiền mặt, chuyển khoản ngân hàng hoặc thẻ tín dụng",
          "Chúng tôi sẽ liên hệ trong vòng 03 giờ sau khi nhận được booking",
        ],
      },

      special: {
        title: "Lưu ý đặc biệt",
        items: [
          "Mỗi chuyến bay: 01 hành khách bay cùng 01 phi công chuyên nghiệp. Phi công kiểm soát toàn bộ chuyến bay, bạn chỉ cần thư giãn, ngắm cảnh và tạo dáng để có ảnh/video đẹp trên không.",
          "Trẻ em trên 3 tuổi được tính là 01 hành khách riêng (không bay cùng bố/mẹ/người thân vì chỉ có 1 ghế).",
        ],
      },

      cancellation: {
        title: "Chính sách huỷ & đổi lịch bay",
        items: [
          "Lịch bay linh động & Hoàn huỷ & đổi lịch miễn phí",
          "Huỷ bay khi thời tiết không thuận lợi & khi khách không sẵn sàng",
          "Khách vui lòng báo đổi/huỷ lịch bay qua email/hotline/Zalo/WhatsApp ít nhất 03 tiếng trước giờ khởi hành",
          "Khách vui lòng thanh toán một số phí phát sinh đã sử dụng nếu huỷ lịch bay trong ngày (nước đã uống, xe đã đón)",
        ],
      },
    },
  } as any,

  contact: {
    title: "Liên hệ với chúng tôi",
    subtitle: "Chúng tôi luôn sẵn sàng hỗ trợ và giải đáp mọi thắc mắc của bạn",
    connectTitle: "Kết nối với chúng tôi",
    connectSubtitle:
      "Chọn kênh liên hệ phù hợp với bạn. Chúng tôi sẵn sàng hỗ trợ 24/7",
    contactNow: "Liên hệ ngay",
    phone: "Điện thoại",
    support247: "Hỗ trợ 24/7",
    address: "Văn phòng",
    workingHours: "Giờ làm việc",
    /** Ngày mở cửa, dùng cho thẻ "Giờ làm việc". */
    openDays: "Thứ 2 - CN",
    officeCity: "Thị trấn Sapa",
    officeProvince: "Lào Cai, Việt Nam",
    social: {
      facebook: "Theo dõi fanpage để cập nhật tin tức mới nhất",
      zalo: "Chat trực tiếp qua Zalo để được tư vấn nhanh",
      whatsapp: "Liên hệ qua WhatsApp 24/7",
      youtube: "Xem video bay dù lượn trên YouTube",
      instagram: "Ảnh và khoảnh khắc bay đẹp nhất trên Instagram",
      tiktokDescription: "Theo dõi kênh TikTok để xem các video bay lượn độc đáo!",
    },
  },

  pilots: {
    title: "Đội ngũ phi công",
    subtitle: "Gặp gỡ những phi công chuyên nghiệp sẽ đồng hành cùng bạn trên bầu trời",
    intro: {
      title: "Chuyên nghiệp - Kinh nghiệm - Tận tâm",
      description:
        "Tất cả phi công của chúng tôi đều được đào tạo bài bản theo tiêu chuẩn quốc tế (APPI & IPPI) và có nhiều năm kinh nghiệm bay. Họ không chỉ là những phi công có số giờ bay dù lượn nhiều nhất Việt Nam mà còn là những người bạn đồng hành nhiệt tình.",
    },
    viewProfile: "Xem hồ sơ",
    nickname: "Biệt danh",
    experience: "Kinh nghiệm",
    flights: "Chuyến bay",
    hours: "Giờ bay",
    contact: "Liên hệ",
    specialties: "Chuyên môn",
    certificates: "Chứng chỉ",
    achievements: "Thành tích",
    funFacts: "Điều thú vị",
    flyingStyle: "Phong cách bay",
    bookWithPilot: "Đặt tour với phi công này",
    gallery: "Thư viện ảnh",
  },

  homestay: {
    badge: "CLUBHOUSE MEBAYLUON",
    title: "Homestay & Café - Bãi Hạ Cánh Dù Lượn",
    slogan: "Bay giữa trời - nghỉ giữa mây - chill giữa ruộng bậc thang",
    callNow: "Gọi đặt phòng",
    viewLocation: "Xem vị trí",
    intro: {
      title: "Chào Mừng Đến Với Clubhouse MEBAYLUON",
      description:
        "Tọa lạc tại Thôn Lìm Thái – Xã Tú Lệ – Tỉnh Lào Cai, cách Hà Nội khoảng 250 km (tầm 5 tiếng đi xe), khu homestay xinh xắn của chúng tôi nằm ngay chân đèo Khau Phạ – một trong “tứ đại đỉnh đèo” nổi tiếng của Việt Nam. Đây là điểm dừng chân lý tưởng cho những ai muốn nghỉ ngơi trước khi chinh phục hành trình hơn 40 km leo đèo Khau Phạ để khám phá trọn vẹn cung đường đèo Khau Phạ (Tú Lệ - Mù Cang Chải) hùng vĩ. Đặc biệt, homestay nằm trong bãi hạ cánh dù lượn, nên chỉ cần nhâm nhi tách cà phê là bạn đã có thể ngắm nhìn những cánh dù rực rỡ lướt trên bầu trời.",
      location: "Vị trí đắc địa",
      traditional: "Nhà sàn truyền thống",
      traditionalDesc: "Vật liệu gỗ tự nhiên, thân thiện môi trường",
      cafe: "Cà phê & Nhà hàng",
      cafeDesc: "View suối và ruộng bậc thang tuyệt đẹp",
    },
    rooms: {
      title: "Các loại phòng",
      subtitle: "Chọn phòng phù hợp với nhu cầu của bạn",
      capacity: "Sức chứa",
      adults: "người lớn",
      children: "trẻ em dưới 5 tuổi",
      bookNow: "Đặt phòng ngay",
      priceTypes: {
        "per-guest": "/khách/đêm",
        "per-room": "/phòng/đêm",
        "whole-home": "/đêm",
      },
      singleRoom: {
        name: "Phòng giường đơn",
        description: "Có 2 phòng, tối đa 1 người lớn và 1 trẻ em dưới 5 tuổi, gồm 1 đệm đơn",
      },
      coupleAtticSingle: {
        name: "Phòng áp mái đơn",
        description: "Có 3 phòng áp mái đơn, mỗi phòng ở tối đa 1 người lớn hoặc 1 người lớn và 1 trẻ em dưới 5 tuổi.",
      },
      coupleAtticDouble: {
        name: "Phòng áp mái đôi",
        description: "1 phòng áp mái đôi duy nhất, tối đa 2 người lớn hoặc 2 người lớn và 1 trẻ em dưới 5 tuổi.",
      },
      doubleRoom: {
        name: "Phòng giường đôi",
        description:
          "Có 2 phòng lớn, phù hợp cho 1 gia đình nhỏ gồm 2 người lớn và 2 trẻ em dưới 5 tuổi, 1 đệm đôi lớn, view suối.",
      },
      dormitory: {
        name: "Phòng Cộng Đồng",
        description: "1 phòng lớn - ở tối đa 20 đệm đơn/sàn",
      },
      wholeHomeSmall: {
        name: "Phòng Gia Đình",
        description: "1 phòng lớn - ở tối đa 5 người/phòng",
      },
      wholeHomeLarge: {
        name: "Nguyên Nhà Sàn",
        description: "Toàn bộ phần sàn cộng đồng, phòng đôi, đơn, các phòng áp mái. Ở tối đa 35 khách",
      },
    },
    features: {
      "en-suite": "Vệ sinh khép kín",
      breakfast: "Bao gồm ăn sáng",
      wifi: "WiFi miễn phí",
      view: "View đẹp",
      "handmade-tea": "Trà handmade",
      "attic-view": "Phòng áp mái",
      "mountain-view": "View núi non",
      "family-friendly": "Phù hợp gia đình",
      "shared-space": "Không gian chung",
      "budget-friendly": "Giá phải chăng",
      "exclusive-use": "Sử dụng riêng",
      "all-facilities": "Đầy đủ tiện nghi",
      "group-friendly": "Phù hợp nhóm",
      "large-group": "Nhóm lớn",
    },
    cafe: {
      title: "Cà phê & Ẩm thực",
      subtitle: "Không gian Cà phê - Nhà hàng ngay tầng trệt, view suối và ruộng bậc thang",
      categories: {
        drinks: "Đồ uống",
        alcohol: "Đặc sản Tú Lệ",
        food: "Món ăn",
      },
      specialNote: "Nhận đặt tiệc",
      specialNoteDesc:
        "Nhận đặt tiệc nhóm, team building. BBQ tập thể hoặc có thể sử dụng bếp nấu món ăn theo nhu cầu.",
    },
    amenities: {
      title: "Tiện ích & Dịch vụ",
      subtitle:
        "Không gian Cà phê – Nhà hàng sở hữu tầm nhìn mở hướng ra bãi hạ dù lượn, ruộng bậc thang, bản làng dân tộc và dòng suối trong lành",
      list: {
        "free-handmade-tea": "Trà handmade miễn phí",
        "free-parking": "Đỗ xe miễn phí",
        "free-wifi": "WiFi miễn phí",
        "shared-bathroom": "4 nhà vệ sinh & 3 nhà tắm",
        "bbq-area": "Khu BBQ",
        campfire: "Đốt lửa trại",
        karaoke: "Hát karaoke",
        "swimming-pool": "Hồ bơi nước suối",
        "camping-area": "Khu camping",
        "team-building-space": "Sân team building 8000m²",
        "trekking-tours": "Tour trekking",
        paragliding: "Bay dù lượn",
        "flycam-service": "Chụp flycam",
      },
    },
    location: {
      title: "Vị trí & Trải nghiệm",
      description:
        "Ngay bãi hạ cánh dù lượn - trung tâm hoạt động của phi công chuyên nghiệp. Khách có thể giao lưu cùng cộng đồng bay, chiêm ngưỡng những cánh dù lượn lơ lửng trên bầu trời.",
      address: "Địa chỉ",
      fromHanoi: "Từ Hà Nội",
      nearby: "Địa điểm gần đây",
    },
    cta: {
      title: "Sẵn sàng đặt phòng?",
      subtitle: "Liên hệ ngay để được tư vấn và đặt phòng với giá tốt nhất",
      bookOnline: "Đặt phòng online",
    },
  },
};