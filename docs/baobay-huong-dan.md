...
Đường dẫn: **mebayluon.com/baobay**
...
- Cấu hình từng điểm ở `/baobay/admin`: giờ chốt báo cáo + webhook bảng Sheets
...
|---|---|---|
| **Phi công** | `/baobay/phi-cong` | Số chuyến + mã vé đã bay; dịch vụ gia tăng (flycam, 360, cờ đỏ, kéo cờ — **chỉ số lượng, mã vé tuỳ chọn**); khách ngoại giao; thu/chi trong ngày |
| **Điều phối bay** | `/baobay/dieu-phoi` | Số khách, vé xuất/thu về, dải mã vé, vé huỷ/dời lịch, flycam, 360, cờ đỏ, bay kéo cờ, khách ngoại giao, tiền mặt + CK, chi cho khách |
| **Camera man** | `/baobay/camera` | Số chuyến quay flycam (+ mã vé nếu ghi được), chi tiêu |
| **Kế toán tổng hợp** | `/baobay/chot-ngay` | Số tổng chốt ngày, duyệt chi tiêu, duyệt lệch, bấm **Chốt ngày** |

Kế toán còn hai trang xem: `/baobay/tong-hop` (theo kỳ, tải CSV) và
`/baobay/bao-cao-thang` (mỗi phi công một khối, cột ngày 1–31 + "đến hôm nay" +
...
- **Phạt nộp muộn**: phi công chốt báo cáo LẦN ĐẦU sau giờ quy định (admin đặt ở
  /baobay/admin, mặc định 20:00, đổi là hiệu lực ngay) bị ghi phạt **200.000đ/lần**.
...
- **Tiền đang giữ và đưa cho quản lý**: mọi trang nhân sự (phi công, điều phối,
  ...
  *Thu hộ* là các dòng tick **THU** trong sổ thu/chi (khách trả tiền tại bãi, thu
  ...
  `/baobay/admin` khi cần. Khai xong là **trừ ngay** khỏi
  ...
  Bên `/baobay/admin`, khung **"Tiền nhân sự giao quản lý"** cho quản trị nhìn
...
Vào **`/admin/baobay`** (menu "Nhân sự báo bay", cần đăng nhập quản trị website):
...
qua các API phía nhân viên (/api/baobay/*).
...
Số liệu tự chảy sang bảng tính (docs/baobay-apps-script.md — bản
`baobay-multispot-v6`):
...
- **Mỗi điểm bay một bảng tính riêng** — khai webhook của từng điểm ở `/baobay/admin`.
...
- **Chuyển từ bản một điểm sang đa điểm**: chạy một lần
  `node scripts/baobay/migrate-spots.mjs` — bỏ chỉ mục duy nhất cũ
...
```
MONGODB_URI="$(grep '^MONGODB_URI=' .env.local | cut -d= -f2-)" node scripts/baobay/demo-data.mjs seed
MONGODB_URI="$(grep '^MONGODB_URI=' .env.local | cut -d= -f2-)" node scripts/baobay/demo-data.mjs clear
```
